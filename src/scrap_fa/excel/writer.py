from enum import IntEnum, auto
from typing import Any

from openpyxl.cell import Cell
from openpyxl.styles import Alignment, Font
from openpyxl.worksheet.worksheet import Worksheet

from src.gui import ProgressBar
from src.pelicula import URL_FILM_ID, Pelicula

from .read_sample import read_sample
from .read_watched import read_data


class ExcelColumns(IntEnum):
    Id = auto()
    Mia = auto()
    FA = auto()
    Duracion = auto()
    Visionados = auto()
    Varianza_FA = auto()
    Prop_aprobados_FA = auto()
    FA_redondeo = auto()
    Diferencia = auto()
    Diferencia_abs = auto()
    Me_ha_gustado = auto()
    Mia_ruido = auto()
    FA_ruido = auto()
    Mia_rees = auto()
    FA_rees = auto()

    def __str__(self) -> str:
        return f"Tabla1[{self.name}]"


class Writer:

    def __init__(self, worksheet: Worksheet):
        # Barra de progreso
        self.bar = ProgressBar()

        # Hoja de excel
        self.worksheet = worksheet

    def write_sample(self, sample_size: int) -> None:
        # Creo una barra de progreso
        bar = ProgressBar()

        # Itero hasta que haya leído todas las películas
        for row, film_data in enumerate(read_sample()):
            write_in_excel(self.worksheet, row, film_data)

            # Actualizo la barra de progreso
            bar.update((row + 1) / sample_size)

            # Si ya tengo suficientes películas, salgo del bucle
            if row > sample_size:
                break

    def write_watched(self, id_user: int):
        # Inicializo la fila actual en la que estoy escribiendo
        bar = ProgressBar()
        for index, (film_data, progress) in enumerate(read_data(id_user)):
            write_in_excel(self.worksheet, index, film_data)
            bar.update(progress)


def write_in_excel(worksheet: Worksheet, line: int, film: Pelicula):

    # La enumeración empezará en 0,
    # pero sólo escribimos datos a partir de la segunda linea.
    line = line + 2

    # En la primera columna guardo la id para poder reconocerla
    set_cell_value(worksheet, line, ExcelColumns.Id,
                   film.titulo, film_id=film.id)

    # La votación del usuario la leo desde fuera
    # no puedo leer la nota del usuario dentro de la ficha
    if film.user_note:
        set_cell_value(worksheet, line, ExcelColumns.Mia,
                       film.user_note)
        set_cell_value(worksheet, line, ExcelColumns.Mia_ruido,
                       f"={ExcelColumns.Mia}+RAND()-0.5")
        set_cell_value(worksheet, line, ExcelColumns.Mia_rees,
                       f"=({ExcelColumns.Mia}-1)*10/9")

    if film.duracion:
        # dejo la casilla en blanco si no logra leer ninguna duración de FA
        set_cell_value(worksheet, line, ExcelColumns.Duracion,
                       film.duracion)

    if film.nota_FA:
        # dejo la casilla en blanco si no logra leer ninguna nota de FA
        set_cell_value(worksheet, line, ExcelColumns.FA,
                       film.nota_FA)
        set_cell_value(worksheet, line, ExcelColumns.FA_redondeo,
                       f"=ROUND({ExcelColumns.FA}*2, 0)/2")
        set_cell_value(worksheet, line, ExcelColumns.Diferencia,
                       f"={ExcelColumns.Mia}-{ExcelColumns.FA}")
        set_cell_value(worksheet, line, ExcelColumns.Diferencia_abs,
                       f"=ABS({ExcelColumns.Diferencia})")
        set_cell_value(worksheet, line, ExcelColumns.Me_ha_gustado,
                       f"=IF({ExcelColumns.Diferencia}>0,1,0.1)")
        set_cell_value(worksheet, line, ExcelColumns.FA_rees,
                       f"=({ExcelColumns.FA}-1)*10/9")
        set_cell_value(worksheet, line, ExcelColumns.FA_ruido,
                       f"={ExcelColumns.FA}+(RAND()-0.5)/10")

    if film.votantes_FA:
        # dejo la casilla en blanco si no logra leer ninguna votantes
        set_cell_value(worksheet, line, ExcelColumns.Visionados,
                       film.votantes_FA)

    if film.desvest_FA:
        set_cell_value(worksheet, line, ExcelColumns.Varianza_FA,
                       film.desvest_FA)
        set_cell_value(worksheet, line, ExcelColumns.Prop_aprobados_FA,
                       film.prop_aprobados)


def set_cell_value(worksheet: Worksheet, line: int, col: ExcelColumns, value: Any,
                   *, film_id: int | None = None):

    # Obtengo un objeto celda
    cell: Cell = worksheet.cell(row=line, column=col)
    # Le asigno el valor
    cell.value = value

    # Configuramos el estilo de la celda atendiendo a su columna
    # visionados. Ponemos punto de millar
    if col == ExcelColumns.Visionados:
        cell.number_format = '#,##0'
    # booleano mayor que
    elif col == ExcelColumns.Me_ha_gustado:
        cell.number_format = '0'
        cell.font = Font(name='SimSun', bold=True)
        cell.alignment = Alignment(horizontal='center', vertical='center')
    # Nota del usuario más el ruido
    elif col == ExcelColumns.Mia_ruido:
        cell.number_format = '0.0'
    # Nota de FA más el ruido
    elif col in (ExcelColumns.FA,
                 ExcelColumns.FA_ruido):
        cell.number_format = '0.00'
    # Varianza de los votos en FA
    elif col == ExcelColumns.Varianza_FA:
        cell.number_format = '0.00'
    # Proporción de aprobados
    elif col == ExcelColumns.Prop_aprobados_FA:
        cell.number_format = '0.00%'
    # reescala
    elif col in (ExcelColumns.Mia_rees,
                 ExcelColumns.FA_rees,
                 ExcelColumns.Diferencia,
                 ExcelColumns.Diferencia_abs):
        cell.number_format = '0.00'
    # Nombre de la película con un hipervínculo
    elif col == ExcelColumns.Id:
        if film_id is None:
            raise ValueError
        # Añado un hipervínculo a su página
        cell.style = 'Hyperlink'
        cell.hyperlink = URL_FILM_ID(film_id)
        # Fuerzo el formato como texto
        cell.number_format = '@'
